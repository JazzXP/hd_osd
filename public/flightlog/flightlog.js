...
const { FlightLogIndex } = require("./flightlog_index");
const {
  FlightLogParser,
...
function FlightLog(logData) {
  const ADDITIONAL_COMPUTED_FIELD_COUNT = 23 /** attitude + PID_SUM + PID_ERROR + RCCOMMAND_SCALED + MOTOR_LEGACY **/,
    that = this,
  ...
  this.getStats = function (logIndex) {
    ...
    // Just modify the raw stats constiable to add this field, the parser won't mind the extra field appearing:
    if (rawStats.frame.S) {
  ...
  function getChunksInIndexRange(startIndex, endIndex) {
    ...
    for (let chunkIndex = startIndex; chunkIndex <= endIndex; chunkIndex++) {
      let chunkStartOffset, chunkEndOffset;
  ...
  function injectComputedFields(sourceChunks, destChunks) {
    ...
    let motor = [
      fieldNameToIndex["motor[0]"],
      fieldNameToIndex["motor[1]"],
      fieldNameToIndex["motor[2]"],
      fieldNameToIndex["motor[3]"],
      fieldNameToIndex["motor[4]"],
      fieldNameToIndex["motor[5]"],
      fieldNameToIndex["motor[6]"],
      fieldNameToIndex["motor[7]"],
    ];
    ...
    for (
      ...
      if (!destChunk.hasAdditionalFields) {
        ...
        const chunkIMU = new IMU(sourceChunks[sourceChunkIndex].initialIMU);
          ...
          if (
            ...
            destFrame[fieldIndex++] =
              rcCommand[AXIS.YAW + 1] !== undefined
                ? that.rcCommandRawToThrottle(srcFrame[rcCommand[AXIS.YAW + 1]])
                : 0;
          }
  ...
  this.getSmoothedChunksInTimeRange = function (startTime, endTime) {
    let sourceChunks, resultChunks, chunkAlreadyDone, allDone;
    const timeFieldIndex =
      FlightLogParser.prototype.FLIGHT_LOG_FIELD_INDEX_TIME;
      ...
      for (let fieldIndex in fieldSmoothing) {
        ...
        mainLoop: for (
          ...
          for (
             ...
             */
            while (
              (leftFrameIndex > 0 || leftFrameIndex === 0) &&
              leftChunkIndex > 0
            ) {
            ...
            while (
              ...
              while (
                ...
                if (
                  rightFrameIndex ===
                  sourceChunks[rightChunkIndex].frames.length
                ) {
                  rightFrameIndex = 0;
                  rightChunkIndex++;
              ...
              if (
                centerFrameIndex ===
                sourceChunks[centerChunkIndex].frames.length
              ) {
                centerFrameIndex = 0;
                centerChunkIndex++;
...
FlightLog.prototype.rcMotorRawToPctEffective = function (value) {
  ...
  const minValue =
    this.isDigitalProtocol() && this.getSysConfig().dynamic_idle_min_rpm > 0
      ? this.getSysConfig().digitalIdleOffset
      : this.getSysConfig().motorOutput[0];
  return Math.min(
    ((value - minValue) / (this.getSysConfig().motorOutput[1] - minValue)) *
      100.0,
    100.0
  );
};
